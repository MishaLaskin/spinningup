...
import gym
import time
import spinup.algos.misha.ppo.core as core
...
def ppo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    if proc_id() == 0:
        hyperparameter_defaults = dict(
                ...
                vf_lr=vf_lr, lam=lam, target_kl=target_kl
                )
                

        if args.sweep:

            wandb.init(config=hyperparameter_defaults,project='ppo_sweep_'+domain_name+'_'+task_name)
            config = wandb.config
        ...
        if (epoch % save_freq == 0) or (epoch == epochs-1):
            ...
            if proc_id()==0:
                ...
                if args.video:
                    ...
                    wandb.log({"video": wandb.Video(video, fps=30, format="gif")},step=epoch)
...
if __name__ == '__main__':
    ...
    if args.dmc:
        ppo(lambda : dmc2gym.make(domain_name=args.domain_name, task_name=args.task_name, seed=args.seed), 
                actor_critic=core.MLPActorCritic,
                ac_kwargs=dict(hidden_sizes=[args.hid]*args.l), gamma=args.gamma, 
                clip_ratio=args.clip_ratio,pi_lr=args.pi_lr,vf_lr=args.vf_lr,lam=args.lam,target_kl=args.target_kl,
                seed=args.seed, steps_per_epoch=args.steps, epochs=args.epochs,
                logger_kwargs=logger_kwargs,sweep=args.sweep,
                video=args.video,domain_name=args.domain_name,task_name=args.task_name)
    else:
        ppo(lambda : gym.make(args.env), actor_critic=core.MLPActorCritic,
            ac_kwargs=dict(hidden_sizes=[args.hid]*args.l), gamma=args.gamma, 
            clip_ratio=args.clip_ratio,pi_lr=args.pi_lr,vf_lr=args.vf_lr,lam=args.lam,target_kl=args.target_kl,
            seed=args.seed, steps_per_epoch=args.steps, epochs=args.epochs,
            logger_kwargs=logger_kwarg,sweep=args.sweeps,
            video=args.video,domain_name=domain_name,task_name=task_name)